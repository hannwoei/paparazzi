...
static void vertical_ctrl_optical_flow_cb(uint8_t sender_id __attribute__((unused)), uint32_t stamp, int16_t flow_x, int16_t flow_y, int16_t flow_der_x, int16_t flow_der_y,
		uint8_t quality, float size_divergence, float fps);
...
{
	// messages.xml needs to be updated when CHECK_CPU_TIME_DIV is defined
...
static void div_ctrl_telem_send(struct transport_tx *trans, struct link_device *dev)
{
  pprz_msg_send_DIV_CTRL(trans, dev, AC_ID,
		  ...
		  &Div_landing.div_igain, &Div_landing.err_Z, &Div_landing.div_f, &Div_landing.ground_div,
		  &stabilization_cmd[COMMAND_THRUST], &Div_landing.thrust, &fb_cmd,
		  &Div_landing.fps, &Div_landing.stamp, &Div_landing.n_corner, &Div_landing.n_track);
}
#endif
#endif

void divergence_landing_init(void);
void divergence_landing_run(bool in_flight);
