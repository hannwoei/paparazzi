...
struct opticflow_t opticflow_v;                      ///< Opticflow calculations
...
//  set_nice_level(10);
  ...
  while (video_thread.is_running) {
    ...
    opticflow_calc_frame(&opticflow_v, &temp_state, &img, &temp_result);
...
void video_thread_init(void)
{
  ...
  opticflow_calc_init(&opticflow_v, vid->w, vid->h);
  opticflow_got_result = FALSE;
