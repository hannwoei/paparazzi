 ...
 */
/**
 * @file "modules/computer_vision/cv_blob_tracking.c"
 * @author H. W. Ho
 * Find a colored item and track it by adjusting roll and pitch and height
 */

#include "modules/computer_vision/cv_blob_tracking.h"
...
#ifndef BLOB_TRACKING_FPS
#define BLOB_TRACKING_FPS 0       ///< Default FPS (zero means run at camera fps)
#endif
PRINT_CONFIG_VAR(BLOB_TRACKING_FPS)
...
uint8_t cv_blob_tracking_reset;
uint8_t cv_blob_tracking_type;

int geofilter_length = 5;
int marker_size = 18;
int record_video = 0;

volatile uint32_t blob_tracking = 0;
...
struct image_t *cv_blob_tracking_func(struct image_t *img);
struct image_t *cv_blob_tracking_func(struct image_t *img)
{
...
void cv_blob_tracking_init(void)
{
...
//  color_cr_max = 150;
  ...
  cv_blob_tracking_reset = 0;

//  georeference_init();

  cv_add_to_device(&BLOB_TRACKING_CAMERA, cv_blob_tracking_func, BLOB_TRACKING_FPS);
  cv_add_to_device(&BLOB_TRACKING_CAMERA, cv_marker_func, BLOB_TRACKING_FPS);
  cv_add_to_device(&BLOB_TRACKING_CAMERA, cv_window_func, BLOB_TRACKING_FPS);
}

void cv_blob_tracking_periodic(void)
{
...
void cv_blob_tracking_event(void)
{
  switch (cv_blob_tracking_type) {
    case 1:
      blob_enabled = true;
...
//    printf("Found %d %d \n", x, y);
    ...
    cam.px = x; // x / 2;
    cam.py = y; // y / 2;
    cam.f = 400; //TODO: focal length
    cam.h = 720; // ar.drone 2.0 front camera resolution
    cam.w = 1280;
...
//  georeference_init();
//  record_video = 1;
//  cv_blob_tracking_type = 3;
//}
//void start_vision_land(void)
//{
//  georeference_init();
//  record_video = 1;
//  cv_blob_tracking_type = 2;
//}
//void stop_vision(void)
//{
//  georeference_init();
//  record_video = 0;
//  cv_blob_tracking_type = 0;
//}
